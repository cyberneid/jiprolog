...
{
    // I termini che valgono priorita' 0 comunque sia dichiarato il loro
    // funtore principale. Sono due casi, e la ISO li tiene distinti:
    //
    //   6.3.4.1  un termine fra parentesi          X = (a ; b)
    //   6.3.3    un composto in notazione funzionale   X = ;(a, b)
    //
    // La priorita' ricalcolata dal funtore non li distingue da a ; b scritto
    // con l'operatore, e senza questa memoria il controllo sugli operandi
    // boccia il kernel: xio.pl scrive EOS = (not), e jipkernel.txt scrive
    // '$system': @>(X, Y), dove @> vale 700 e il ramo destro di ':' arriva a
    // 600. Sono entrambi legali, e senza il registro sarebbero entrambi
    // rifiutati.
    //
    // Identita' e non uguaglianza: gli atomi sono internati e aliaserebbero,
    // i composti sono costruiti freschi, e qui dentro finiscono solo composti.
    private final IdentityHashMap<Object, Object> m_priorityZero = new IdentityHashMap<Object, Object>();
...
//              System.out.println("nextoken " + tok.m_strToken);
                ...
                // ISO 6.3.1.2: un segno seguito DIRETTAMENTE da un numerale
                // denota la costante negativa (o positiva), non l'operatore
                // prefisso applicato al numero. Vale solo in posizione di
                // operando - stack vuoto oppure operatore in cima - altrimenti
                // in "a-1" il meno verrebbe letto come segno invece che come
                // operatore infisso.
                //
                // Il tokenizer emette il whitespace come token a se', quindi
                // "il token successivo e' un numero" significa gia' "non c'era
                // layout in mezzo": e' cosi' che "- 7" resta -(7) mentre "-7"
                // e' l'intero negativo.
                if((tok.m_nType == PrologTokenizer.TOKEN_ATOM || tok.m_nType == PrologTokenizer.TOKEN_SPECIAL_ATOM)
                        && (tok.m_strToken.equals("-") || tok.m_strToken.equals("+"))
                        && (lastObj == null || lastObj instanceof Operator))
                {
                    final PrologTokenizer.Token nextTok = m_tokenizer.getNextToken();

                    if(nextTok != null)
                    {
                        if(nextTok.m_nType == PrologTokenizer.TOKEN_NUMBER)
                        {
                            sign = tok.m_strToken;
                            tok = nextTok;
                        }
                        ...
                        {
                            m_tokenizer.pushBackToken(nextTok);
                        }
                    }
                }
                    ...
                    case PrologTokenizer.TOKEN_SINGLETON:
                        {
                            ...
                            else if(tok.m_strToken.equals("("))
                            {
                                ...
                                if(lastObj instanceof Atom)// || lastObj instanceof Operator)
                                {
                                    ...
                                    m_priorityZero.put(funct, funct);
                                        ...
                                        else //if(lastObj instanceof Operator)
                                        {
                                            ...
                                            else  // stack not empty
                                            {
                                                ...
                                                else if(lastOp.getPrefix() != null)
                                                {
                                                    ...
                                                    {
                                                        // lastOp era stato spinto come prefisso scommettendo che
                                                        // un operando seguisse. Arriva invece curOp, che un
                                                        // termine non lo puo' iniziare: l'operando non c'e' mai
                                                        // stato, e lastOp era un atomo. Vale anche se lastOp ha
                                                        // una forma infissa - anche quella vorrebbe qualcosa alla
                                                        // propria destra.
                                                        //
                                                        // Spingere curOp sopra il prefisso pendente saltava del
                                                        // tutto il confronto di precedenze, e resolveStack
                                                        // srotolava poi da destra: "X = not ; c" diventava
                                                        // =(X, ;(not,c)), un operando a 1100 sotto un operatore a
                                                        // 700 che ne ammette 699. Convertito lastOp in atomo e
                                                        // rimesso in lastObj, il giro successivo del while lo
                                                        // tratta come un termine qualsiasi e passa dal ramo che
                                                        // le precedenze le confronta.
                                                        final Atom atom1 = Atom.createAtom(lastOp.getName());
                                                        termStack.pop();
                                                        termStack.push(atom1);
                                                        lastObj = atom1;
                                                    }
...
//                                                        System.out.println("LasOP infix " + lastOp.m_strName + "CurOp Prefix " + curOp.m_strName);
    ...
    private final int operandPriority(final Object obj)
    {
        ...
        if(m_priorityZero.containsKey(obj))
            return 0;
    ...
    private final PrologObject resolveOperator(Stack termStack, Operator op) throws JIPSyntaxErrorException
    {
        ...
        if(op.isPrefix())//prefix
        {
            ...
            // layout, per cui "- 7" diventava l'intero -7 invece di -(7) e
            // "-2 ** 2" diventava -(**(2,2)) invece di **(-2,2).
            // Ora il segno viene riconosciuto sul token, in translateTerm.
